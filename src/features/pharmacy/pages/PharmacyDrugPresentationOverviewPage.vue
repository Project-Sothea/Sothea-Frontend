  ...
  <div class="container max-w-5xl px-4 mx-auto sm:px-8 table">
    <div class="py-8">
      ...
      <div class="flex flex-row justify-between w-full mb-4">
        <h2 class="text-3xl leading-tight" style="color: black">Presentation Overview</h2>
        <div class="space-x-2">
          <router-link
            to="/pharmacy"
      ...
      <!-- Presentation Header (bigger/clearer) -->
      <div class="mb-8">
        <div class="rounded-2xl border bg-indigo-50 p-5 sm:p-6">
          <div class="leading-snug">
            ...
            <div class="mt-3 text-sm sm:text-base text-gray-800">
              <span class="font-semibold">Notes:</span> {{ pres?.notes || '–' }}
            </div>
          </div>
        </div>
      </div>
...
import { getPresentation } from '../api/drug'
import { fmtDispenseUnit, fmtStrengthWithRoute, type ISODateString } from '../types/Util'
import type { DrugPresentationView } from '../types/Drug'
import { fmtDate } from '../types/Util'

/** Route param */
const route = useRoute()
const presentationId = Number(route.params.presentationId)

/** State */
const tab = ref<'batches' | 'locations'>('batches')
const searchTerm = ref('')

const pres = ref<DrugPresentationView>()
...
async function fetchData() {
  ...
  const presStock = await getPresentation(presentationId)
  pres.value = presStock.presentation
  batches.value = presStock.batches
}

/** Header label builders */
const headerTitle = computed(() => pres.value?.drugName || '—')

const headerSubtitle = computed(() => fmtStrengthWithRoute(pres.value))

const headerMeta = computed(() => fmtDispenseUnit(pres.value))
