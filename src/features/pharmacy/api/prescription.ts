...
import type { LineAllocation, Prescription, PrescriptionLine, PrescriptionLinePostData, PrescriptionPostData, SetAllocationsPostData } from "../types/Prescription"
...
export async function deletePrescription(id: number) {
    const { data } = await http.delete<Prescription>(`/prescriptions/${id}`)
    return data
}
...
export async function createLine(prescriptionId: number, newLine: PrescriptionLinePostData) {
    const { data } = await http.post<PrescriptionLine>(`/prescriptions/${prescriptionId}/lines`, newLine)
    return data
}
...
export async function setLineAllocations(lineId: number, newLines: SetAllocationsPostData) {
    const { data } = await http.put<LineAllocation[]>(`/prescriptions/lines/${lineId}/allocations`, newLines)
    return data
}
