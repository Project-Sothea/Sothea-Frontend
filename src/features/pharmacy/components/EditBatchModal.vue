                ...
                >
                  ...
                  <div class="col-span-3 sm:col-span-1">
                    <button
                      ...
                      class="w-full h-[42px] mt-1 px-2 text-red-600 border border-red-600 rounded hover:bg-red-50 transition-colors flex items-center justify-center"
...
function addLocation() {
  form.value.locations.push({ location: '', quantity: 0 })
  // New locations don't have an ID, so we don't add to originalLocationIds
}

function removeLocation(idx: number) {
  if (form.value.locations.length > 1) {
    form.value.locations.splice(idx, 1)
    // Remove the original ID mapping for this location
    const newIds = new Map<number, number>()
    originalLocationIds.value.forEach((id, formIdx) => {
      if (formIdx < idx) {
        newIds.set(formIdx, id)
      } else if (formIdx > idx) {
        newIds.set(formIdx - 1, id)
      }
    })
...
async function handleSubmit() {
  ...
  saving.value = true
  try {
    // Update batch with locations in a single call
    // Include location IDs when they exist (for matching/updating), omit for new locations (for creating)
    // Backend rules:
    // 1. Match locations via id. If location/qty changes, always allow update
    // 2. If location has unknown id (not in payload), create it
    // 3. If location exists in system but not in payload, delete it (unless used by prescription - backend will error)
    await updateBatch(props.batch.id, {
      ...
      locations: form.value.locations.map((loc, idx) => {
        const locationId = originalLocationIds.value.get(idx)
        // If we have an original ID, include it for matching/updating
        // If not, omit it so backend knows to create a new location
        return {
          location: loc.location,
          quantity: loc.quantity,
