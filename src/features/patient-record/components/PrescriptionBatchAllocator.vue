<template>
  ...
  <div class="mt-2 flex items-center justify-between">
    <div class="text-sm text-gray-700">
      ...
      <span
        v-if="anyDirty"
        class="ml-2 inline-flex items-center text-xs px-2 py-0.5 rounded-full bg-amber-50 text-amber-700 border border-amber-200"
      >
        Unsaved changes
      </span>
    </div>
    ...
    >
      ...
      <div class="flex items-center gap-2">
        <button
          ...
          @click="decrement(i)"
          :disabled="readonly || sel.allocatedQuantity <= 1"
        >
          −
        </button>
        <input
        ...
        <button
          ...
          class="px-2 py-1 border rounded text-sm"
          @click="increment(i)"
          :disabled="readonly || sel.allocatedQuantity >= sel.available"
        >
          +
        </button>
        <button
      ...
      >
        ...
        <button
          ...
          :disabled="
            readonly ||
            opt.available <= 0 ||
            isSelected(opt.batchLocationId) ||
            remainingRequested <= 0
          "
          @click="add(opt)"
        >
...
const selectedById = computed(
  () =>
    new Map(selectedOptions.value.map((s) => [s.batchLocationId, Number(s.allocatedQuantity) || 0]))
)
...
const totalAllocated = computed(() =>
  selectedOptions.value.reduce((s, r) => s + (Number(r.allocatedQuantity) || 0), 0)
)
...
function hydrateSelections(from?: LineAllocation[]) {
  ...
  baselineAlloc.value = new Map(
    selectedOptions.value.map((s) => [s.batchLocationId, Number(s.allocatedQuantity) || 0])
  )
...
async function save() {
  ...
  const post = selectedOptions.value
    .map((r) => ({
      batchLocationId: Number(r.batchLocationId),
      quantity: Number(r.allocatedQuantity)
    }))
...
const filteredOptions = computed<Option[]>(() => {
  ...
  return list.filter(
    (o) =>
      (o.batchNumber?.toLowerCase().includes(q) ?? false) || o.location.toLowerCase().includes(q)
  )
})
...
function normalize(i: number) {
  ...
  if (!row) return
  row.allocatedQuantity = Math.max(
    1,
    Math.min(Number(row.allocatedQuantity || 0), maxAllocFor(row.batchLocationId))
  )
}
...
function increment(i: number) {
  const row = selectedOptions.value[i]
  if (!row) return
  row.allocatedQuantity = Math.min(
    (row.allocatedQuantity || 0) + 1,
    maxAllocFor(row.batchLocationId)
  )
}
