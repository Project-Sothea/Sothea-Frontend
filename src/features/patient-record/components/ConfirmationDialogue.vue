    ...
    <div v-if="open" class="fixed inset-0 z-50 flex items-center justify-center" role="dialog" aria-modal="true">
      ...
      <div class="relative z-10 w-full max-w-md mx-4 rounded-2xl bg-white shadow-xl" @click.stop>
        ...
        <div class="px-5 py-4 border-t flex justify-end gap-2">
          <button class="px-4 py-2 rounded border" @click="$emit('cancel')">{{ cancelText }}</button>
          <button class="px-4 py-2 rounded text-white bg-red-600 hover:bg-red-700" @click="$emit('confirm')">{{ confirmText }}</button>
        </div>
      </div>
    </div>
  </Teleport>
...
function handleBackdrop() { if (props.closeOnBackdrop) emit('cancel') }
</script>
